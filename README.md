...
│   ├── train_phi3_omenortep.py         # Fine-Tuning local (alternativa ao Colab)
...
│   └── Omenortep_AI.ipynb              # Notebook oficial para Google Colab
...
- **Hiperparâmetros de referência:**
  ...
  - `lora_alpha`: 16
  - `learning_rate`: 2e-4
- **Notebook:** `scripts/Omenortep_AI.ipynb` (upload para o Google Colab).
...
- **Procedimento:**
  ...
  3. Registrar o modelo: `ollama create omenortep-v1 -f models/Modelfile`.
  4. Rodar via CLI: `ollama run omenortep-v1`.
- **Interface (Open WebUI):** `docker compose up -d` — acesse em [http://localhost:3000](http://localhost:3000) e selecione `omenortep-v1` no seletor de modelos.
...
- [x] Logar todas as métricas no **MLflow/DagsHub** (experimento `omenortep-benchmark`).
- [x] Definir **trigger de retreinamento** (`SIMILARITY_TRIGGER_THRESHOLD = 0.50`).

### 4. Deploy (Fase 4)
- [x] Instalar Ollama no Windows (Nativo).
- [x] Registrar e rodar o modelo via `ollama create omenortep-v1 -f models/Modelfile`.
...
- [x] Validar Open WebUI em [http://localhost:3000](http://localhost:3000) com `omenortep-v1:latest` respondendo em Português (BR).
